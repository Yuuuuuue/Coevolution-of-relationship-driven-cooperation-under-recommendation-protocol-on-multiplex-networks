...
b_values = np.arange(1, 2.05, 0.05)  # b的取值从1到2，每次增加0.05
...
for b in b_values:
        ...
        for u in G_game.nodes():
            ...
            if neighbors_u:
                ...
                for v in neighbors_u:
                    # 如果u和v在关系层有直接连边，则以概率p添加边
                    if G_coop.has_edge(u, v):
                        if random.random() < p:
                            G_game.add_edge(u, v)
                    # 如果v不是x，v与u的关系值大于γ，且x与v之间在博弈层尚未添加边
                    if v != x and edge_weights.get((u, v), 0) > gamma and not G_coop.has_edge(x, v):
                        if random.random() < p:
                            G_game.add_edge(x, v)
    ...
    results.append((b, mean_cooperator_ratios))

# 打印最终结果
for b, mean_cooperator_ratios in results:
    print(f"b = {b}, Mean Cooperator Ratios for each m value:", mean_cooperator_ratios)
