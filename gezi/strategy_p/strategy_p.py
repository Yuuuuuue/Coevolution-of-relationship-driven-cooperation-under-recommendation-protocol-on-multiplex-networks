...
# 固定参数设置
b = 1.5  # 背叛诱惑
m = 0.5  # 第一层有边的条件下第二层连边概率
d = 0.05  # 权值变化
K = 0.1  # 噪声参数
gamma = 0.2  # 阈值

R = np.array([[1, 0], [b, 0]])  # 收益矩阵
p_values = [0.75,1]  # p的取值
...
for p in p_values:
        ...
        for edge in G_game.edges():
            ...
            if strategies[node1] == 0 and strategies[node2] == 0:
                if G_coop.has_edge(node1, node2):
                    edge_weights[(node1, node2)] = min(edge_weights[(node1, node2)] + d, 1)
            elif strategies[node1] == 1 and strategies[node2] == 1:
                if G_coop.has_edge(node1, node2):
                    edge_weights[(node1, node2)] = max(edge_weights[(node1, node2)] - d, 0)
    ...
    print(f"Mean Cooperator Ratio in Stable State for p = {p}:", mean_stable_cooperator_ratio)

    # 保存每个p值的策略结果到Excel
    strategy_results = {'Node': list(G_coop.nodes()), 'Strategy': strategies}
    df = pd.DataFrame(strategy_results)
    df.to_excel(f"strategies_p_{p}.xlsx", index=False)
