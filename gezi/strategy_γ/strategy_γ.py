...
for gamma in gamma_values:
    ...
    print(f"Mean Cooperator Ratio in Stable State for γ = {gamma}:", mean_stable_cooperator_ratio)

    # 保存每个b值的策略结果到Excel
    strategy_results = {'Node': list(G_coop.nodes()), 'Strategy': strategies}
    df = pd.DataFrame(strategy_results)
    df.to_excel(f"strategies_γ_{gamma}.xlsx", index=False)
